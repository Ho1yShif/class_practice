...
from dfs_helpers import get_path, offsets, is_legal_pos, read_maze
from stack import Stack

# My version of DFS
def dfs(maze, start, goal):
	# Change– assigned a direction list up here for readability
	# instead of putting the list directly in the for loop
	...
	while not stack.is_empty():
		...
		for dir in directions:
			row_offset, col_offset = offsets[dir]
			neighbor = (current[0] + row_offset, current[1] + col_offset)
			# Change– added parenthesis around these conditions
			if (is_legal_pos(maze, neighbor)) and (neighbor not in predecessors):
				stack.push(neighbor)
				predecessors[neighbor] = current
		# Removed unnecessary "return None" at the end of the function
